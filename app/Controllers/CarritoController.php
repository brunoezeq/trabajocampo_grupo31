...
class CarritoController extends BaseController
{
    ...
    public function comprarCarrito()
    {
        ...
        if (empty($itemsCarrito)) {
            return redirect()->to('verCarrito')
                             ->with('mensaje', 'El carrito est� vac�o.');
        }
        ...
        // Validar stock usando el m�todo que ahora est� en CarritoService
        try {
        ...
        try { 
            ...
            $resultado = $ventaController->registrarVenta($itemsCarrito, $clienteId, $medioPagoId);

            if ($resultado === false) {
                return redirect()->to('mostrarCarrito')
                                 ->with('mensaje', 'Error al registrar la compra, int�ntelo nuevamente.');
            }
        } catch (ValidationException $ve) {
            return redirect()->to('mostrarCarrito')
                             ->withInput()
    ...
    public function eliminarItemCarrito($id_producto)
    {
       ...
       if (!$productoEliminado) {
           return redirect()->to('verCarrito')
                            ->with('mensaje', 'No se encontr� el item en el carrito.');
       }
       return redirect()->to('verCarrito')
                        ->with('mensaje', 'Se elimin� el item del carrito.');
    }
